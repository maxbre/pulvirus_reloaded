...
if(!exists("df_sval")) {
  con <- dbConnect(PostgreSQL(), host = "10.158.102.164", user = "srv-pulvirus", password = "pulvirus#20", dbname = "pulvirus")
  
  sql <- "SELECT * FROM riepilogo_valide"
  rs <- dbSendQuery(con, sql)
  df_sval <- fetch(rs, n = -1)
  
  sql <- "SELECT * FROM riepilogo_non_valide"
  rs <- dbSendQuery(con, sql)
  df_nval <- fetch(rs, n = -1)
  dbDisconnect(con)
