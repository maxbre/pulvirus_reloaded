...
outdir <- "~/R/pulvirus/presentazione/validazione/"
...
for (i in rdatas) {
  ...
  eu_code <- stringr::str_split(basename(i), "_")[[1]][2]
...
sapply(my_df, function(x) { as.vector(x) }) %>% 
  write.table(file = glue::glue("{outdir}/validazione_{pltnt}_adj.csv"), sep = ";", row.names = FALSE)
